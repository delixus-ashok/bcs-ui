const main = async () => {
  ...
  const BcsToken = await hre.ethers.getContractFactory("BcsToken");
  const bcsToken = await BcsToken.deploy();

  await bcsToken.deployed();

  console.log("BcsToken deployed to:", bcsToken.address);
}
